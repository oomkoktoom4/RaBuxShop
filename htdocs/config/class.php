...
class classweb_bypluem{
	...
	public function topup($link_topup){
        ...
        $postData = [
            'mobile' => "0802513173",
            'voucher_hash' => $link_topup
        ];
		//CURL
		$ch = curl_init();
        curl_setopt($ch, CURLOPT_URL,"https://gift.truemoney.com/campaign/vouchers/$link_topup/redeem");
        ...
        curl_close($ch);
        $result = json_decode($data_response);

		/*
		$web_config = $this->web_config();
		$phone = "0802954799" // เดะมาแก้นะ ผมลองให้ก่อน
		$curl = curl_init();
		...
		*/
		...
		if ($codestatus =="SUCCESS"){
			...
			try {
				...
				echo json_encode(array('status'=>"success",'message'=>"เติมเงินเรียบร้อย"));
			} catch (Exception $e) {
				echo json_encode(array('status'=>"error",'message'=>$e->getmessage()));
			}
		}else{
